...
{
  MMCMSG(smc_host->mmc, "smc %d err, cmd %d, %a%a%a%a%a%a%a%a%a%a%a\n",
...
static int mmc_set_mclk(struct sunxi_mmc_host* mmchost, u32 clk_hz)
{
  ...
  if (div > 128) {
    m = 1;
    n = 0;
    MMCINFO("%a: source clock is too high, clk %d, src %d!!!\n",
...
static unsigned mmc_get_mclk(struct sunxi_mmc_host* mmchost)
{
  ...
  if (src == 0)
    ...
    MMCINFO("%a: wrong clock source %d\n",__func__, src);
  }
...
static unsigned mmc_config_delay(struct sunxi_mmc_host* mmchost)
{
  ...
  else if (mode == SUNXI_MMC_TIMING_MODE_4)
  {
    ...
    MMCDBG("%a: spd_md:%d, freq:%d, odly: %d; sdly: %d; dsdly: %d\n", __FUNCTION__, spd_md, freq, odly, sdly, dsdly);
  }
...
static int mmc_calibrate_delay_unit(struct sunxi_mmc_host* mmchost)
{
  ...
  MMCINFO("start %a, don't access device...\n", __FUNCTION__);
  ...
  if ((mmchost->timing_mode == SUNXI_MMC_TIMING_MODE_1)
    ...
    MMCINFO("%a: mmc %d wrong timing mode: 0x%x\n",
  ...
  if ((mmchost->mmc_no == 2) || (mmchost->mmc_no == 3)) {
  ...
  if (result) {
    ...
    MMCINFO("%a: cal sample delay fail\n", __FUNCTION__);
  }
  if (mmchost->mmc_no == 2) {
    rval = readl(&reg->ds_dl);
    result = (rval & SDXC_CalDly) >> 8;
    ...
    if (result) {
      ...
      MMCINFO("%a: cal data strobe delay fail\n", __FUNCTION__);
    }
  }
#endif  /* FPGA_PLATFORM */
  return 0;
...
static void mmc_set_ios(struct mmc *mmc)
{
  ...
  #else //modified by Leson 20150728
...
static int mmc_trans_data_by_dma(struct mmc *mmc, struct mmc_data *data)
{
  ...
  OSAL_CacheRangeFlush(buff, (UINT32)byte_cnt, CACHE_CLEAN_FLUSH_D_CACHE_REGION);
        ...
        struct mmc_data *data)
{
  ...
  if (cmd->resp_type & MMC_RSP_BUSY) {
    ...
    if ((cmd->cmdidx == MMC_CMD_ERASE)
      || ((cmd->cmdidx == MMC_CMD_SWITCH)
        &&(((cmd->cmdarg>>16)&0xFF) == EXT_CSD_SANITIZE_START)))
      MMCINFO("%a: cmd %d wait rsp busy 0x%x us \n",__FUNCTION__,
