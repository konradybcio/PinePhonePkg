...
u64 lldiv(u64 dividend, u32 divisor);

//LIST_HEAD(mmc_devices);
struct mmc* mmc_devices[MAX_MMC_NUM];
         ...
         lbaint_t blkcnt)
{
  ...
  if (mmc_send_cmd(mmc, &cmd, &data)) {
    MMCMSG(mmc, "read block failed, %a %d\n", __FUNCTION__, __LINE__);
    return 0;
  }
  if (blkcnt > 1) {
    cmd.cmdidx = MMC_CMD_STOP_TRANSMISSION;
...
ulong mmc_bread(int dev_num, lbaint_t start, lbaint_t blkcnt, void *dst)
{
  ...
  do {
    ...
    if(mmc_read_blocks(mmc, dst, start, cur) != cur) {
      MMCMSG(mmc, "block read failed, %a %d\n", __FUNCTION__, __LINE__);
      return 0;
    }
    blocks_todo -= cur;
    start += cur;
...
{
  struct mmc_cmd cmd;
  UINT64 timeout = 1000; //use ns, 1000ms
  //uint start;
  int err;
...
static int mmc_set_capacity(struct mmc *mmc, int part_num)
{
  ...
  mmc->block_dev.lba = (lbaint_t)(mmc->capacity >> 9); //lldiv(mmc->capacity, mmc->read_bl_len);
  mmc->lba = mmc->block_dev.lba;
  return 0;
}
...
static int mmc_startup(struct mmc *mmc)
{
  ...
  if (!IS_SD(mmc) && (mmc->version >= MMC_VERSION_4)) {
    ...
    if (!err && (ext_csd[EXT_CSD_REV] >= 2)) {
      ...
      capacity = *(u32*)(ext_csd+EXT_CSD_SEC_CNT);
  ...
  MMCINFO("%a -- 0x%02x-%02x-%02x-%02x-%02x\n", mmc->block_dev.product,
...
struct mmc *mmc_create(const struct mmc_config *cfg, void *priv)
{
  ...
  if (cfg == NULL || cfg->ops == NULL || cfg->ops->send_cmd == NULL ||
    ...
    MMCINFO("%a: input error\n", __FUNCTION__);
    return NULL;
  }

  mmc = malloc(1*sizeof(*mmc));
  if (mmc == NULL) {
    MMCINFO("%a: request memory error\n\n", __FUNCTION__);
    return NULL;
  }
  ...
  mmc->block_dev.if_type = 6;//IF_TYPE_MMC is 6
...
int mmc_start_init(struct mmc *mmc)
{
  ...
  //memset(priv_info, 0xff, sizeof(struct boot_sdmmc_private_info_t ));
...
__ERROR_END:
  MMCINFO("fdt err returned %a\n", fdt_strerror(ret));
  return ;
  #endif
  printf("##not need to update mmc para to kernel\n");
}
...
int mmc_init_boot(struct mmc *mmc)
{
  ...
  work_mode = SunxiGetBootWorkMode();
 ...
 */
uint64_t lldiv(uint64_t dividend, uint32_t divisor)
{
