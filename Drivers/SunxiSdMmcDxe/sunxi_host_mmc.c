...
static void mmc_get_para_from_fex(int sdc_no)
{
  ...
  if (sdc_no == 0)
  {
    ...
    ret = script_parser_fetch("card0_boot_para", "sdc_odly_50M_ddr", &rval, 1);
    ...
    ret = script_parser_fetch("card0_boot_para", "sdc_sdly_50M_ddr", &rval, 1);
    ...
    ret = script_parser_fetch("card0_boot_para", "sdc_freq", &rval, 1);
    ...
    ret = script_parser_fetch("card0_boot_para", "sdc_b0p", &rval, 1);
    ...
    if (cfg->platform_caps.sample_mode == MAUNAL_SAMPLE_MODE)
    {
      for (imd=0; imd<MAX_SPD_MD_NUM; imd++)
      {
        for (ifreq=0; ifreq<MAX_CLK_FREQ_NUM; ifreq++)
        {
          ...
          ret = script_parser_fetch("card0_boot_para", ctmp, &rval, 1);
          //ret = fdt_getprop_u32(working_fdt, nodeoffset, ctmp, (uint32_t*)(&rval));
          if (ret<0) {
            MMCDBG("get sdc0 %a fail.\n", ctmp);
            ...
            MMCINFO("get sdc0 %a 0x%x for %d-%a %d.\n", ctmp,
    ...
    */
    ...
    ret = gpio_request_simple("sdc1", NULL);
    if(ret)
    {
    ...
    ret = gpio_request_simple("card2_boot_para", NULL);
    if(ret)
    {
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_io_1v8", &rval, 1);
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_odly_50M_ddr", &rval, 1);
    //ret = fdt_getprop_u32(working_fdt,nodeoffset,"sdc_odly_50M_ddr", (uint32_t*)(&rval));
    if (ret<0) {
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_sdly_50M_ddr", &rval, 1);
    //ret = fdt_getprop_u32(working_fdt,nodeoffset,"sdc_sdly_50M_ddr", (uint32_t*)(&rval));
    if (ret<0) {
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_freq", &rval, 1);
    //ret = fdt_getprop_u32(working_fdt,nodeoffset,"sdc_freq", (uint32_t*)(&rval));
    if (ret<0) {
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_b0p", &rval, 1);
    //ret = fdt_getprop_u32(working_fdt,nodeoffset,"sdc_b0p", (uint32_t*)(&rval));
    if (ret<0) {
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_tm4_win_th", &rval, 1);
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_tm4_r_cycle", &rval, 1);
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_tm4_hs200_max_freq", &rval, 1);
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_tm4_hs400_max_freq", &rval, 1);
    ...
    for (i=0; i<MAX_EXT_FREQ_POINT_NUM; i++)
    {
      ...
      ret = script_parser_fetch("card2_boot_para", ctmp, &rval, 1);
      //ret = fdt_getprop_u32(working_fdt,nodeoffset, ctmp, (uint32_t*)(&rval));
      if (ret<0) {
        MMCDBG("get sdc2 %a fail.\n", ctmp);
        cfg->platform_caps.tm4_tune_ext_freq[i] = 0x0;
      } else {
        MMCINFO("get sdc2 %a 0x%x.\n", ctmp, rval);
        if ((rval & 0xff) >= 25)
      ...
      for (imd=0; imd<MAX_SPD_MD_NUM; imd++)
      {
        for (ifreq=0; ifreq<MAX_CLK_FREQ_NUM; ifreq++)
        {
          ...
          if (ret<0) {
            ...
            cfg->platform_caps.odly_spd_freq[imd*MAX_CLK_FREQ_NUM + ifreq] = rval&0x1;
            MMCINFO("get sdc2 %a 0x%x for %d-%a %d.\n", ctmp,
              cfg->platform_caps.odly_spd_freq[imd*MAX_CLK_FREQ_NUM + ifreq], imd, spd_name[imd], ifreq);
          }
    ...
    /* speed mode caps */
    ret = script_parser_fetch("card2_boot_para", "sdc_dis_host_caps", &rval, 1);
    ...
    ret = script_parser_fetch("card2_boot_para", "sdc_kernel_no_limit", &rval, 1);
  ...
  /* fmax, fmax_ddr */
    ...
    MMCINFO("%a: input sdc_no error: %d\n", __FUNCTION__, sdc_no);
  }
...
static void mmc_get_para_from_dtb(int sdc_no)
{
  ...
  if (sdc_no == 0)
  {
    ...
    nodeoffset = fdt_path_offset(working_fdt, prop_path);
    if (nodeoffset < 0) {
      MMCINFO("can't find node \"%a\",will add new node\n", prop_path);
      goto __ERRRO_END;
    }
  }
  else
...
extern struct mmc* mmc_devices[MAX_MMC_NUM];
...
int sunxi_sun50iw1p1_mmc_init(int sdc_no)
{
  ...
  if (host->pdes == NULL) {
    MMCINFO("%a: get mem for descriptor failed\n", __FUNCTION__);
    return -1;
  }
  ...
  if (host->mmc == NULL) {
    MMCINFO("%a: register mmc %d failed\n", __FUNCTION__, sdc_no);
    return -1;
  } else
  {
    MMCINFO("%a: register mmc %d ok\n", __FUNCTION__, sdc_no);
    mmc_devices[host->cfg.host_no] = host->mmc;
  }
