...
sunxi_dma_init(VOID)
{
  ...
  for(i=0;i<SUNXI_DMA_MAX;i++)
  {
    ...
    DmaAllocateBuffer(EfiBootServicesData,EFI_SIZE_TO_PAGES(sizeof(sunxi_dma_start_t)),(void **)&dma_channal_source[i].config);
...
#if defined(CONFIG_ARCH_SUN8IW3P1)||defined(CONFIG_ARCH_SUN8IW5P1)||defined(CONFIG_ARCH_SUN8IW6P1)||defined(CONFIG_ARCH_SUN8IW8P1)
  {
    ...
    reg_val = *(volatile UINTN *)(SUNXI_DMA_BASE + 0x20);
    reg_val &= ~7;
    reg_val |= 4;
    *(volatile UINTN *)(SUNXI_DMA_BASE + 0x20) = reg_val;
  }
#endif
...
static    EFI_STATUS    EFIAPI  sunxi_dma_disable_int(UINT32 hdma);

static    EFI_STATUS    EFIAPI  sunxi_dma_free_int(UINT32 hdma);
/*
************************************************************************************************************
*
*                                             function
...
sunxi_dma_exit(VOID)
{
  ...
  for(i=0;i<SUNXI_DMA_MAX;i++)
  {
    ...
    {
      hdma = (UINT32)&dma_channal_source[i];
  ...
  for(i=0;i<SUNXI_DMA_MAX;i++)
  {
    DmaFreeBuffer(EfiBootServicesData,(VOID *)dma_channal_source[i].config);
  }
...
STATIC SUNXI_DMA_PROTOCOL SunxiDmaProtocol = {
.Revision = 0x00000001,
.DmaRequest = sunxi_dma_request,
.DmaRelease = sunxi_dma_release,
.DmaSetting = sunxi_dma_setting,
.DmaStart   = sunxi_dma_start,
.DmaStop    = sunxi_dma_stop,
.DmaQueryStatus = sunxi_dma_querystatus,
.DmaInstallInterruptHandle  = sunxi_dma_install_int,
.DmaFreeInterruptHandle = sunxi_dma_free_int,
.DmaEnableInterrupt = sunxi_dma_enable_int,
.DmaDisableInterrupt = sunxi_dma_disable_int
};

EFI_STATUS
SunxiDmaDxeInitialize (
  ...
  )
{
  ...
  DEBUG((DEBUG_INFO,"++%a\n", __func__));
  // Get the interrupt protocol for later use
  ...
  Status = gBS->InstallMultipleProtocolInterfaces(&ImageHandle, &gSunxiDmaProtocolGuid, &SunxiDmaProtocol, NULL);
